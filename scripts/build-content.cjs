...
async function buildContent() {
  await fs.mkdir(path.join(__dirname, '..', 'src', 'content', 'posts'), { recursive: true })
  let pages = []
  ...
  for (const page of pages) {
    const pageProperties = await renderNotionPageProperties(page, propertyRenderers)
    log({ pageProperties })
    ...
    if (!pageProperties.excerpt) {
      const plainTextBlocks = await renderNotionBlocks(page.children, plainTextRenderers)
      ...
      let excerpt = content.slice(0, 150).trim().split(' ').slice(0, -1).join(' ') + '...'
